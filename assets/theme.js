...
class LineItemQuantity extends HTMLElement {
  ...
  _bindEvents() {
    // Input change
    this.addEventListener("change", this._onQuantityChanged.bind(this));
    // Remove link
    this.addEventListener("click", this._onRemoveLinkClicked.bind(this));
    // Plus/Minus buttons
    this.querySelectorAll(".quantity-selector__button").forEach(btn => {
      if (btn.getAttribute("aria-label")?.includes("increase")) {
        btn.addEventListener("click", this._onPlusClicked.bind(this));
      } else if (btn.getAttribute("aria-label")?.includes("decrease")) {
        btn.addEventListener("click", this._onMinusClicked.bind(this));
      }
    });
  ...
  _onMinusClicked(event) {
    ...
    input.value = newVal >= parseInt(input.min || 0) ? newVal : input.min || 0;
    input.dispatchEvent(new Event("change", { bubbles: true }));
  }
