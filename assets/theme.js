...
class LineItemQuantity extends HTMLElement {
  constructor() {
    super();
    // Bind events
    this._onQuantityChanged = this._onQuantityChanged.bind(this);
    this._onRemoveLinkClicked = this._onRemoveLinkClicked.bind(this);
    this._onPlusClicked = this._onPlusClicked.bind(this);
    this._onMinusClicked = this._onMinusClicked.bind(this);

    // Listen to input change and remove link
    this.addEventListener("change", this._onQuantityChanged);
    this.addEventListener("click", this._onRemoveLinkClicked);

    // Listen for plus/minus buttons
    this.querySelectorAll(".quantity-selector__button").forEach(btn => {
      if (btn.getAttribute("aria-label")?.includes("increase")) {
        btn.addEventListener("click", this._onPlusClicked);
      } else if (btn.getAttribute("aria-label")?.includes("decrease")) {
        btn.addEventListener("click", this._onMinusClicked);
      }
    });

    // Listen for cart updates to patch the input value
    document.addEventListener("cart:change", this._onCartChange.bind(this));
  }

  _onQuantityChanged(event) {
    const target = event.target;
    if (!target.hasAttribute("data-line-key")) return;
    this._changeLineItemQuantity(target.getAttribute("data-line-key"), parseInt(target.value));
  }

  _onPlusClicked(event) {
    event.preventDefault();
  ...
  _onMinusClicked(event) {
    event.preventDefault();
    const input = this.querySelector(".quantity-input");
    const newVal = parseInt(input.value) - parseInt(input.step || 1);
    input.value = newVal >= parseInt(input.min || 0) ? newVal : input.min;
    input.dispatchEvent(new Event("change", { bubbles: true }));
  }

  _onRemoveLinkClicked(event) {
    const target = event.target;
    if (target.tagName !== "A" || !target.href.includes("/cart/change")) return;
    event.preventDefault();
  ...
  async _changeLineItemQuantity(lineKey, targetQuantity) {
    ...
    try {
      ...
      if (!response.ok) {
        this._showError(cartContent.description);
        return;
      }

      // Update input value, do not replace HTML
      const input = this.querySelector(".quantity-input");
      ...
      lineItem?.dispatchEvent(new CustomEvent("line-item:change", {
        bubbles: true,
        detail: { quantity: input.value, cart: cartContent }
      }));
  ...
  _showError(message) {
    const lineItemInfo = this.closest(".line-item, tr")?.querySelector(".line-item__info");
    if (!lineItemInfo) return;

    // Remove previous errors
    lineItemInfo.querySelectorAll('[role="alert"]').forEach(el => el.remove());
    ...
    lineItemInfo.insertAdjacentHTML("beforeend", `<p class="text-with-icon text-error text-xs" role="alert">${errorSvg} ${message}</p>`);
  }
